...
public class ArchitectureGraphBuilder {
    ...
    public Graph addEdgesByMatrix2(Graph g, Map<Pair<String,String>, Double> matrix, String parentArchitecture) {
    ...
    public Graph addEdgesByMatrix(Graph g, Map<Pair<String,String>, Double> matrix, String parentArchitecture) {
        ...
        while (unconnected.size() > 0) {
            ...
            for (Pair<String,String> arch: matrix.keySet()) {
                double thisScore = matrix.get(arch);
                if (!Tuple.get1(arch).equals(Tuple.get2(arch)) && unconnected.contains(Tuple.get2(arch))) {
                    // if we haven't already added its opposite
                    if (!toConnect.contains(Tuple.from(architectureNodeId.get(Tuple.get2(arch)), architectureNodeId.get(Tuple.get1(arch))))) {
                        if (thisScore > maxscore) {
                            targetsToRemove.clear();
                            targetsToRemove.add(Tuple.get1(arch));
                            targetsToRemove.add(Tuple.get2(arch));
                            toConnect.clear();
                            toConnect.add(Tuple.from(architectureNodeId.get(Tuple.get1(arch)), architectureNodeId.get(Tuple.get2(arch))));
                            maxscore = thisScore;
